...
## Required parameters
flags.DEFINE_string(
    "data_dir", '/content/drive/My Drive/bert-ner-xiandai/tmp',
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "bert_config_file", '/content/drive/My Drive/bert-ner-xiandai/bert-model/bert_config.json',
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", 'NER', "The name of the task to train.")

flags.DEFINE_string("vocab_file", '/content/drive/My Drive/bert-ner-xiandai/bert-model/vocab.txt',
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "output_dir", '/content/drive/My Drive/bert-ner-xiandai/output-1',
    "The output directory where the model checkpoints will be written.")

## Other parameters

flags.DEFINE_string(
    "init_checkpoint", '/content/drive/My Drive/bert-ner-xiandai/output-1/model.ckpt-6749',
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("loss"):
        ...
        # with tf.variable_scope("loss"):
        trans = tf.get_variable("transitions",shape=[10, 10],initializer=initializers.xavier_initializer())
        if labels is None:
                 return None, trans
        else:
            log_likelihood, trans = tf.contrib.crf.crf_log_likelihood(inputs=logits,tag_indices=labels,transition_params=trans,sequence_lengths=lengths)
            loss = tf.reduce_mean(-log_likelihood)
            predict, _ = crf.crf_decode(potentials=logits, transition_params=trans, sequence_length=lengths)
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids,predicts):
               accuracy = tf.metrics.accuracy(label_ids,predicts)
               precision = tf.metrics.precision(label_ids, predicts)
               recall = tf.metrics.recall(label_ids, predicts)
               f = tf_metrics.f1(label_ids, predicts,10)
               return {
                   "eval_accuracy_token": accuracy,
                   "eval_precision_token": precision,
                   "eval_recall_token": recall,
                   "eval_f": f,
            ...
            output_spec = tf.estimator.EstimatorSpec(
                ...
                export_outputs={'output': tf.estimator.export.PredictOutput(predicts)})
        return output_spec
...
def main(_):
  ...
  #
  #
  #         # for line in open(os.path.join(FLAGS.output_dir, "test_prediction.txt"), 'r', encoding='utf-8'):
  # #           for line in
