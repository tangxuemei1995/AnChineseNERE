...
～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～～·
run_NER-1.py : BERT +CRF finetuning runner 现代汉语的语料 tmp中的语料
run_NER.py :BERT finetuning runner 现代汉语的语料 tmp中的语料
run_classfier.py：BERT用于分类的源代码
run_NER-guji-ceshi.py: 用于古籍标点
ckpt_restore_hook.py :用于读取初始模型
merics_bert.py:用于评测便标点和断句
modeling.py： bert模型
